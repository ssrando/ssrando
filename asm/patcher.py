...
def apply_dol_patch(self, dol, patch):
  for org_address, patchlet in patch.items():
    ...
    if org_address >= free_space_start:
      ...
      dol.write_data(write_and_pack_bytes, org_address, new_bytes, "B"*len(new_bytes))
...
def apply_rel_patch(self, rel, rel_name, patches):
  ...
  for org_address, patchlet in patches.items():
    ...
    if offset >= free_space_start:
      apply_free_space_patchlet_to_rel(self, rel, rel_name, offset, new_bytes, relocations)
    else:
      ...
      if relocations:
        ...
        add_relocations_to_rel(self, rel, rel_name, rel_section_index, offset_into_section, relocations)
...
def add_free_space_section_to_main_dol(main_dol, new_bytes):
  ...
  main_dol.write_data(write_and_pack_bytes, ORIGINAL_FREE_SPACE_RAM_ADDRESS, new_bytes, "B"*len(new_bytes))
...
def apply_free_space_patchlet_to_rel(self, rel, file_path, offset, new_bytes, relocations):
...
def add_relocations_to_rel(self, rel, file_path, rel_section_index, offset_into_section, relocations):

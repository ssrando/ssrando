...
section_names = {1: 'text0', 2: 'data0', 3: 'data1', 4: 'data2', 5: 'data3', 6: 'uninitialized'}

rel_arc = U8File.parse_u8(BytesIO((Path('../actual-extract') / 'DATA' / 'files' / 'rels.arc').read_bytes()))
...
for relpath in rel_arc.get_all_paths():
    ...
    for sec_num, section in enumerate(rel.sections):
        if sec_num >= 7 or sec_num == 0:
            ...
            rel_info[section_names[sec_num]] = {'offset': section.offset, 'length': section.length}
...
with open('rel_info.yaml','w') as f:
    yaml.dump(all_rel_info, f, Dumper=yaml.CDumper, default_flow_style=False, sort_keys=False)

with open('free_space_start_offsets2.txt', 'w') as f:
    yaml.dump(free_start_offsets, f, Dumper=yaml.CDumper, default_flow_style=False, sort_keys=False)

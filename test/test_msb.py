...
def get_all_msb():
    all_msb={}
    for arc in (Path(__file__).parent.parent/'actual-extract'/'DATA'/'files').glob('*/Object/*/*.arc'):
        with open(arc,'rb') as f:
            eventfile=sslib.U8File.parse_u8(BytesIO(f.read()))
        msb_paths = filter(lambda x: x.endswith('.msbf') or x.endswith('.msbt'), eventfile.get_all_paths())

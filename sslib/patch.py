...
class Patcher:
    ...
    def set_room_patch(self, stage: str, room: int, patchfunc: Callable[[ParsedBzs], ParsedBzs]):
    ...
    def do_patch(self):
        ...
        stages_layer_to_patch.update((stage, 0) for stage, room in self.room_patches.items())
        ...
        for stagepath in (self.actual_extract_path/'DATA'/'files'/'Stage').glob('*/*_stg_l*.arc.LZ'):
            ...
            if not (stage, layer) in stages_layer_to_patch:
                ...
                for roomid, patchfunc in self.room_patches[stage].items():
                    roomarc = U8File.parse_u8(BytesIO(stageu8.get_file_data(f'rarc/{stage}_r{roomid:02}.arc')))
        ...
        needed_eventfiles = set(x[0] for x in self.event_patches.keys()) # first letter determines which file to use

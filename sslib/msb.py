...
def buildMSB(msb: ParsedMsb) -> bytes:
    ...
    for seg_id, seg_data in msb.items():
        ...
        if seg_id == 'type':
            continue
        if seg_id == 'FLW3':
            body += struct.pack('>hh', len(seg_data['flow']), len(seg_data['branch_points']))
            body += b'\x00' * 12
            for flow in seg_data['flow']:
                body += struct.pack('>bbhhhhhhh', FLOWTYPES[flow['type']], flow['subType'], 0,
                    flow['param1'], flow['param2'], flow['next'], flow['param3'], flow['param4'], flow['param5'])
